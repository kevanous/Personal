...
describe "Coding documents as Responsive, also creating a summary report, view, search, imaging profile and dtSearch index " do
	...
	context "Create a new search and imaging profile" do
		it "a new search profile should be created" do
			...
			@automationSmokeTest.createSearchProfile(WorkspaceElements::SavedSearchName, WorkspaceElements::FolderName)
			expect(@automationSmokeTest.savedSearchResultCount).to be > 1
		end
	end
	...
	context "Create a new dtSearch index" do
		it "will create a new dtSearch index" do
			@relativityworkspace.TabAccess(ApplicationNames::WorkspaceAdmin)
			@documentdisplay.dtSearchIndexCreate(WorkspaceElements::DtSearchName, WorkspaceElements::SavedSearchName)
			expect(@documentdisplay.dtSearchIndexSuccess).to eq ("Active - Indexed")
			expect(@documentdisplay.dtSearchIndexNewVerify(WorkspaceElements::DtSearchName)).to be true
		end
	end
