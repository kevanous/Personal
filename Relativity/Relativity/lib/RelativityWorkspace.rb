...
class RelativityWorkspace
  ...
  def AnalyticsIndexField(fieldName)
      $browser.ElementByCss('#horizontal-subtabstrip > ul > li > a:contains(%s)'% "Search Indexes").wait_until_present
      ...
      sleep 2
    # Select Analytics Server from Dropdown
  ...
  def AddAnalyticsServer(fieldName)
    RelativityGroupAccess("Server & Agent Management")
    $browser.ElementByCss('#horizontal-subtabstrip > ul > li > a:contains(%s)'% "Servers").wait_until_present.click
    # tab_iFrame.button(:text => "New Resource Server").click
    iframe_main.a(:text=>"New Resource Server").wait_until_present.click
    $browser.text_field(:id =>'_editTemplate__kCuraScrollingDiv_Name_textBox_textBox').set fieldName
  ...
  def AddServerToDefaultResourcePool(serverType, serverName)
    ...
    sleep 2
    # Original id value : ctl00_viewRenderer_itemList_listTable
    # ctl00_ctl00_itemList_listTable - Replace this in the below line for id
    iframe_main.table(:id=>"ctl00_ctl00_itemList_listTable").a(:text=>"Default").click
    sleep 2
  ...
  def getWorkspaceArtifactID
    ...
    if $browser.select(:id=>"_view__wizardTemplate__kCuraScrollingDiv__twoList_ListOneBox").option(:text=>"Case Artifact ID").exists? == true
      ...
      return iframe_main.td(:text=>RelativityWorkspaceItems::WorkspaceNameSmoke).parent.tds[3].text
