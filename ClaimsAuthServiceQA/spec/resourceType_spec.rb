...
describe 'Claims Auth services functionality for ResourceType entity' do
  
  context 'making a GET request for resourceType' do
    before {
      @get_call = @claims_auth.get('ResourceType')
    }
  ...
  context 'making a POST request for resourceType' do
    before {
      @post_call = @claims_auth.post('ResourceType')
      # $resourceType_id = JSON.parse(@post_call[:body])['Id']
    }
  ...
  context 'making a PATCH request for resourceType' do
    before {
      @patch_call = @claims_auth.patch('ResourceType')
    }
  ...
  context 'making a PUT request for resourceType' do
    before {
      @put_call = @claims_auth.put('ResourceType')
    }
    it 'results in a successful update of the entity per the update(put) body in the request' do
      expect(@put_call[:code]).to eq 200
      # Validating that the input body's Type for the request is the same as the output Type
      expect(JSON.parse(@put_call[:body])['Type']).to eq JSON.parse(@put_call[:inputObject])['Type']
    end
  end

  context 'making a DELETE request for resourceType' do
    before {
      @delete_call = @claims_auth.delete('ResourceType')
