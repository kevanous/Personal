...
describe 'Claims Auth services functionality for Policy entity' do
  
  context 'making a GET request for policy' do
    before {
      @get_call = @claims_auth.get('Policy')
    }
  ...
  context 'making a POST request for policy' do
    before {
      @post_call = @claims_auth.post('Policy')
      # $policy_id = JSON.parse(@post_call[:body])['Id']
    }
    it 'results in successful response for request and a new resource is created' do
      ...
      expect(JSON.parse(@post_call[:body])['ConditionOperator']).to eq JSON.parse(@post_call[:inputObject])['ConditionOperator']
    end
  end

  context 'making a PATCH request for policy' do
    before {
      @patch_call = @claims_auth.patch('Policy')
    }
    it 'results in a successful update of the entity per the update(patch) body in the request' do
      ...
      expect(JSON.parse(@patch_call[:body])['ConditionOperator']).to eq JSON.parse(@patch_call[:inputObject])['ConditionOperator']
    end
  end

  context 'making a PUT request for policy' do
    before {
      @put_call = @claims_auth.put('Policy')
    }
    it 'results in a successful update of the entity per the update(put) body in the request' do
      ...
      expect(JSON.parse(@put_call[:body])['ConditionOperator']).to eq JSON.parse(@put_call[:inputObject])['ConditionOperator']
  end
  end

  context 'making a DELETE request for policy' do
    before {
      @delete_call = @claims_auth.delete('Policy')
