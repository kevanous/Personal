...
describe 'Claims Auth services functionality for ManagementAccount entity' do
  
  context 'making a GET request for managementAccount' do
    before {
      @get_call = @claims_auth.get('ManagementAccount')
    }
  ...
  context 'making a POST request for managementAccount' do
    before {
      @post_call = @claims_auth.post('ManagementAccount')
      # $managementAccount_id = JSON.parse(@post_call[:body])['Id']
    }
    it 'results in successful response for request and a new resouce is created' do
      ...
      expect(JSON.parse(@post_call[:body])['Account']).to eq JSON.parse(@post_call[:inputObject])['Account']
    end
  end

  context 'making a PATCH request for managementAccount' do
    before {
      @patch_call = @claims_auth.patch('ManagementAccount')
    }
    it 'results in a successful update of the entity per the update(patch) body in the request' do
      ...
      # Validating that the input body's property for the request is the same as the output property
      expect(JSON.parse(@patch_call[:body])['Account']).to eq JSON.parse(@patch_call[:inputObject])['Account']
    end
  end

  context 'making a PUT request for managementAccount' do
    before {
      @put_call = @claims_auth.put('ManagementAccount')
    }
    it 'results in a successful update of the entity per the update(put) body in the request' do
      expect(@put_call[:code]).to eq 200
      # Validating that the input body's Account for the request is the same as the output Account
      expect(JSON.parse(@put_call[:body])['Account']).to eq JSON.parse(@put_call[:inputObject])['Account']
    end
  end

  context 'making a DELETE request for managementAccount' do
    before {
      @delete_call = @claims_auth.delete('ManagementAccount')
      @input_object = JSON.parse(@delete_call[:inputObject])
      @input_id = @input_object['Id'].to_s
      @input_account = @input_object['Account']
      @get_call = @claims_auth.get('ManagementAccount')
    }
    it 'results in the resource being deleted with 204 success code' do
      ...
      expect(@get_call[:body]).not_to include @input_account
    end
  end
end
