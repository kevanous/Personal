...
describe 'Claims Auth services functionality for AppClaim entity' do
  
  context 'making a GET request for appClaim' do
    before {
      @get_call = @claims_auth.get('AppClaim')
    }
  ...
  context 'making a POST request for appClaim' do
    before {
      @post_call = @claims_auth.post('AppClaim')
      # $appClaim_id = JSON.parse(@post_call[:body])['Id']
    }
  ...
  context 'making a PATCH request for appClaim' do
    before {
      @patch_call = @claims_auth.patch('AppClaim')
    }
  ...
  context 'making a PUT request for appClaim' do
    before {
      @put_call = @claims_auth.put('AppClaim')
    }
  ...
  context 'making a DELETE request for appClaim' do
    before {
      @delete_call = @claims_auth.delete('AppClaim')
